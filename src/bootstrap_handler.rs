...
pub fn serialise_contacts(contacts: ::contact::Contacts) -> Vec<u8> {
    let encoded = json::encode(&contacts).unwrap();
    return encoded.into_bytes();
}

pub fn parse_contacts(buffer: Vec<u8>) -> Option<::contact::Contacts> {
...
impl BootstrapHandler {
    fn get_file_path() -> path::PathBuf {
        ...
        let file_path = ::file_handler::user_app_dir().unwrap().join(&name);
        if File::open(&file_path).is_ok() {
            return file_path;
        }

        let file_path = ::file_handler::system_cache_dir().unwrap();
        if File::open(&file_path).is_ok() {
            return file_path;
        }
    ...
    pub fn update_contacts(&mut self, contacts: ::contact::Contacts,
                           prune: ::contact::Contacts) -> io::Result<()> {
    ...
    pub fn read_bootstrap_file(&mut self) -> io::Result<(::contact::Contacts)> {
    ...
    #[allow(dead_code)]
    pub fn oldest_contacts(&mut self, n: usize) -> io::Result<(::contact::Contacts)> {
        ...
        Ok(bootstrap_contacts.iter().rev().map(|contact| contact.clone())
                             .take(n).collect::<::contact::Contacts>())
    }
    ...
    fn write_bootstrap_file(&mut self, mut contacts: ::contact::Contacts) -> io::Result<()> {
    ...
    fn insert_contacts(&mut self, mut contacts: ::contact::Contacts,
                       prune: ::contact::Contacts) -> io::Result<()> {
