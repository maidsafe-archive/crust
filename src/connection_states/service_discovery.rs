...
impl ServiceDiscovery {
    pub fn new(core: &mut Core,
        ...
        let bind_addr = ("0.0.0.0", port).to_socket_addrs().expect("Failed in binding UDP socket")
        ...
        let bound_port = match udp_socket.bind(&bind_addr) {
            Ok(()) => udp_socket.local_addr().expect("Failed get local_addr").port(),
            Err(e) => panic!("Failed in binding address with error {:?}", e),
        };
    ...
    fn enable_listen_for_peers(&mut self, _core: &mut Core, event_loop: &mut EventLoop<Core>) {
    ...
    fn disable_listen_for_peers(&mut self, _core: &mut Core, event_loop: &mut EventLoop<Core>) {
    ...
    fn seek_peers(&mut self, _core: &mut Core, event_loop: &mut EventLoop<Core>) {
...
impl State for ServiceDiscovery {
    ...
    fn write(&mut self, core: &mut Core, event_loop: &mut EventLoop<Core>, data: Vec<u8>) {
        let config: ServiceDiscoveryConfig = if let Ok(msg) = deserialise(&data) {
            msg
        } else {
            return;
        };
        match config {
            ServiceDiscoveryConfig::DisableServiceDiscovery =>
                self.disable_listen_for_peers(core, event_loop),
            ServiceDiscoveryConfig::EnableServiceDiscovery =>
                self.enable_listen_for_peers(core, event_loop),
            ServiceDiscoveryConfig::SeekPeers =>
                self.seek_peers(core, event_loop),
        }
    }
