...
impl From<CrustError> for io::Error {
    fn from(err: CrustError) -> io::Error {
        match err {
            CrustError::Io(e) => e,
            CrustError::MioNotify(e) => {
                match e {
                    mio::NotifyError::Io(e) => e,
                    mio::NotifyError::Full(..) => {
                        io::Error::new(io::ErrorKind::Other, "Mio notify error (channel full)")
                    }
                    mio::NotifyError::Closed(..) => {
                        io::Error::new(io::ErrorKind::Other, "Mio notify error (channel closed)")
                    }
                }
            }
            CrustError::ChannelRecv(e) => io::Error::new(io::ErrorKind::Other, e),
            CrustError::ConfigFileHandler(e) => io::Error::new(io::ErrorKind::Other, e),
            CrustError::ServiceDiscNotEnabled => {
                io::Error::new(io::ErrorKind::Other, "Service discovery not enabled")
            }
            CrustError::Nat(e) => io::Error::new(io::ErrorKind::Other, e),
            CrustError::ServiceDisc(e) => io::Error::new(io::ErrorKind::Other, e),
            CrustError::MioTimer(e) => {
                io::Error::new(io::ErrorKind::Other, format!("Mio timer error: {:?}", e))
            }
            CrustError::PayloadSizeProhibitive => {
                io::Error::new(io::ErrorKind::Other, "Payload size prohibitive")
            }
            CrustError::PeerNotFound(peer_id) => {
                io::Error::new(io::ErrorKind::Other,
                               format!("Peer not found: {:?}", peer_id))
            }
            CrustError::Serialisation(e) => io::Error::new(io::ErrorKind::Other, e),
        }
    }
}
