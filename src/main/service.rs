...
use common::{Context, Core, CoreMessage, Priority, Socket};
...
use main::peer_id;
use main::{Bootstrap, Connect, ConnectionCandidate, ConnectionListener, ConnectionMap, CrustError,
           Event, PeerId};
...
impl Service {
    ...
    pub fn stop_bootstrap(&mut self) -> ::Res<()> {
        self.post(move |mut core, mut el| {
            core.terminate_state(el, BOOTSTRAP_CONTEXT);
        })
    }
    ...
    pub fn connect(&self, our_ci: PrivConnectionInfo, their_ci: PubConnectionInfo) -> ::Res<()> {
        ...
        Ok(try!(self.post(move |core, el| {
            Connect::start(core, el, our_ci, their_ci, cm, our_nh, event_tx);
        })))
    }
...
mod tests {
    ...
    fn connect(service_0: &Service,
        ...
        expect_event!(event_rx_0, Event::NewPeer(res) => {
            let peer_id = unwrap_result!(res);
            assert_eq!(peer_id, service_1.id());
        });

        expect_event!(event_rx_1, Event::NewPeer(res) => {
            assert_eq!(unwrap_result!(res), service_0.id());
        });
