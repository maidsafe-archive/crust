...
use connection_states::establish_connection::EstablishConnection;
...
pub struct Service {
    ...
    connection_map: Arc<Mutex<HashMap<u64, Context>>>,
    _raii_joiner: RaiiThreadJoiner,
}

impl Service {
    fn new(tx: Sender<CrustMsg>) -> Self {
        let event_loop = EventLoop::new().expect("Unable to create event loop");
        let mio_tx = event_loop.channel();

        let raii_joiner = RaiiThreadJoiner::new(thread!("CoreEventLoop", move || {
            let core = Core::new();
            event_loop.run(&core).expect("EventLoop failed to run");
        }));
    ...
    pub fn connect(&mut self, peer_contact_info: SocketAddr) {
        ...
        let cm = self.connection_map.clone();
    ...
    pub fn drop_peer(&mut self, peer_id: u64) {
        ...
        let _ = self.mio_tx.send(Box::new(move |core, event_loop| {
            ...
            state.terminate(&mut core, &mut event_loop);
        }));
    }
    ...
    pub fn send(&mut self, peer_id: u64, data: Vec<u8>) {
        ...
        let _ = self.mio_tx.send(Box::new(move |core, event_loop| {
            let state = core.get_state(&context).expect("State not found").clone();
            state.write(&mut core, &mut event_loop, data);
        }));
    }
}
